...
void dma_request_config(uint32_t periph, uint8_t type, uint8_t index)
{
    ...
    LL_DMA_SetChannelSelection(
        (DMA_TypeDef*)dma_get(periph, type, index), dma_channel_get(periph, type, index), channel);
}
...
void enable_pclock(uint32_t periph_base)
{
    ...
    if (periph_base < APB2PERIPH_BASE) {
        ...
        if (pos == 8) {  // ADC
            if (periph_base == ADC2_BASE)
                pos = 9;
            else if (periph_base == ADC3_BASE)
                pos = 10;
        }
        RCC->APB2ENR |= (1 << pos);
        RCC->APB2ENR;
...
#define PLL_M (HSI_VALUE / 1000000U)
    ...
    PeriphClkInit.PeriphClockSelection =
        RCC_PERIPHCLK_USART1 | RCC_PERIPHCLK_USART2 | RCC_PERIPHCLK_USART3 | RCC_PERIPHCLK_USART6
        | RCC_PERIPHCLK_UART4 | RCC_PERIPHCLK_UART5 | RCC_PERIPHCLK_UART7 | RCC_PERIPHCLK_UART8
        | RCC_PERIPHCLK_I2C1 | RCC_PERIPHCLK_I2C3 | RCC_PERIPHCLK_I2C2 | RCC_PERIPHCLK_I2C4;
...
int gpio_irq_get(uint8_t const pin)
{
    if (10 <= pin)
...
gpio_isr_cb_t gpio_callback[7];

void gpio_isr_set(gpio_isr_cb_t cb, IRQn_Type irqnb)
{
    if (EXTI15_10_IRQn == irqnb)
        gpio_callback[6] = cb;
    else if (EXTI9_5_IRQn == irqnb)
        gpio_callback[5] = cb;
    else
        gpio_callback[irqnb - EXTI0_IRQn] = cb;
}
...
{
    gpio_isr_cb_t cb = gpio_callback[3];
    if (cb)
        cb();
    EXTI->PR = 1 << 3;
}
...
{
    gpio_isr_cb_t cb = gpio_callback[4];
    if (cb)
        cb();
    EXTI->PR = 1 << 4;
}
...
{
    gpio_isr_cb_t cb = gpio_callback[5];
    if (cb)
        cb();
    EXTI->PR = 0x3E0;
}
...
{
    gpio_isr_cb_t cb = gpio_callback[6];
    if (cb)
        cb();
    EXTI->PR = 0xFE00;
}

void FAST_CODE_1 DMA1_Stream0_IRQHandler(void)
{
    USARTx_DMA_handler(4);
}  // UART5 RX
void FAST_CODE_1 DMA1_Stream1_IRQHandler(void)
{
    USARTx_DMA_handler(2);
}  // USART3 RX
void FAST_CODE_1 DMA1_Stream2_IRQHandler(void)
{
    USARTx_DMA_handler(3);
}  // UART4 RX
void FAST_CODE_1 DMA1_Stream3_IRQHandler(void)
{
    USARTx_DMA_handler(2);
}  // USART3 TX
void FAST_CODE_1 DMA1_Stream4_IRQHandler(void)
{
    USARTx_DMA_handler(3);
}  // UART4 TX
void FAST_CODE_1 DMA1_Stream5_IRQHandler(void)
{
    USARTx_DMA_handler(1);
}  // USART2 RX
void FAST_CODE_1 DMA1_Stream6_IRQHandler(void)
{
    USARTx_DMA_handler(1);
}  // USART2 TX
void FAST_CODE_1 DMA1_Stream7_IRQHandler(void)
{
    USARTx_DMA_handler(4);
}  // UART5 TX

// void FAST_CODE_1 DMA2_Stream0_IRQHandler(void) {Error_Handler();} // ADC1 DMA
void FAST_CODE_1 DMA2_Stream1_IRQHandler(void)
{
    USARTx_DMA_handler(5);
}  // USART6 RX
void FAST_CODE_1 DMA2_Stream2_IRQHandler(void)
{
...
void FAST_CODE_1 DMA2_Stream6_IRQHandler(void)
{
    USARTx_DMA_handler(5);
}  // USART6 TX
void FAST_CODE_1 DMA2_Stream7_IRQHandler(void)
{
    USARTx_DMA_handler(0);
}  // USART1 TX
